...
cp /home/vagrant/tz-elk/resources/elasticsearch/config/elasticsearch_aws.yml /etc/elasticsearch/
ipaddr=`ifconfig eth0 | grep 'inet addr:' | cut -d: -f2 | awk '{ print $1}'`
sed -i "s/network.host: 0.0.0.0/network.host: "$ipaddr"/g" /etc/$ES/elasticsearch.yml
